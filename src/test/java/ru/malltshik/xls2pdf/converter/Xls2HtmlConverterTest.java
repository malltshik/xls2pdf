...
public class Xls2HtmlConverterTest {
    ...
    public void convert() throws Exception {
        ...
        String filename = format("%s/%s.html", System.getProperty("java.io.tmpdir"), UUID.randomUUID().toString());
        File target = new File(filename);

        LOGGER.debug("Test target file name is: {} ", filename);

        OutputStream result = new Xls2HtmlConverter(getClass().getClassLoader().getResourceAsStream("test.xlsx"),
